...
export const discoverUsers = asyncHandler(async (req, res) => {
  ...
  const lifeSkills = [
    "Fitness", "Meditation", "Public Speaking",
  ];
  const cookingSkills = [
   "Baking", "Grilling", "Indian Cuisine", "Chinese Cuisine", "Italian Cooking", "Healthy Recipes",
 ...
 "Canva", "Photoshop", "Figma",
  ...
  users.forEach((user) => {
    ...
    } else if (user.skillsProficientAt.some((skill) => artSkills.includes(skill)) &&  artUsers.length < 5) {
      artUsers.push(user);
    } else if (user.skillsProficientAt.some((skill) => musicSkills.includes(skill)) && musicUsers.length < 5) {
      musicUsers.push(user);
    }
    else if (user.skillsProficientAt.some((skill) => lifeSkills.includes(skill)) &&   lifeUsers.length < 5) {
      lifeUsers.push(user);
    }
    else if (user.skillsProficientAt.some((skill) => cookingSkills.includes(skill)) &&  cookUsers.length < 5) {
      cookUsers.push(user);
    }
    else if (user.skillsProficientAt.some((skill) => graphicSkills.includes(skill)) &&  graphicUsers.length < 5) {
      graphicUsers.push(user);
    }
    else if (user.skillsProficientAt.some((skill) => sportsSkills.includes(skill)) &&  sportsUsers.length < 5) {
      sportsUsers.push(user);
    }
    else if (user.skillsProficientAt.some((skill) => programmigSkills.includes(skill)) &&  programUsers.length < 5) {
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { forYou: usersToLearn, artUser: artUsers, musicUser: musicUsers, lifeUser: lifeUsers,cookUser: cookUsers, graphicUser: graphicUsers, sportsUser: sportsUsers,programUser:programUsers, others: otherUsers },
